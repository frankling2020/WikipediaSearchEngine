...
class TestFairnessMetric(unittest.TestCase):
    ...
    def test_nfairr_score_cutoff_3(self):
        # Test calculating NFaiRR using the cut-off of 3
        omega_values_actual_ranked = [0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 1, 1, 0, 1, 0, 0, 0, 1, 0, 0]

        expected_score = 0
        score = nfairr_score(omega_values_actual_ranked, 3)

        self.assertAlmostEqual(score, expected_score, places=3,
                               msg=f"Expected NFaiRR score={expected_score}, but got {score}")

    def test_nfairr_score_cutoff_2(self):
        # Test calculating NFaiRR using the cut-off of 3
        omega_values_actual_ranked = [1, 1, 0, 1, 0, 0, 0, 1, 0, 0, 1, 1, 0, 1, 0, 0, 0, 1, 0, 0]

        expected_score = 1
        score = nfairr_score(omega_values_actual_ranked, 2)
