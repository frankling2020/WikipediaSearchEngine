...
#                print(score)
    ...
    def test_cross_encoder_query_nonexistent_docid(self):
        raw_text_dict = {}
        with open('data.jsonl', 'r', encoding='utf-8') as file:
            for line in file:
                data = json.loads(line)
        ...
        ce_scorer = CrossEncoderScorer(raw_text_dict)
        exp_score = 0
        actual_score = ce_scorer.score(100, 'AI chatbots and vehicles')
        self.assertAlmostEqual(actual_score, exp_score, places=3,
                                 msg=f'Expected score {exp_score} but received {actual_score}')
    
    def test_cross_encoder_query_empty_query(self):
        raw_text_dict = {}
        with open('data.jsonl', 'r', encoding='utf-8') as file:
            for line in file:
                data = json.loads(line)
        ...
        ce_scorer = CrossEncoderScorer(raw_text_dict)
        exp_score = 0
        actual_score = ce_scorer.score(4, '')
        self.assertAlmostEqual(actual_score, exp_score, places=3,
                                 msg=f'Expected score {exp_score} but received {actual_score}')
