...
class TestL2RRanker(unittest.TestCase):
    ...
    def test_prepare_training_data_empty_input(self):
        ...
        X, y, qgroups = l2r.prepare_training_data(query_to_doc_rels)
        self.assertEqual(len(X), 0, "Expected 0 feature vectors, i.e., len(X) from prepare_training_data")
        self.assertEqual(len(y), 0, "Expected 0 relevance scores, i.e., len(y) from prepare_training_data")
        self.assertEqual(len(qgroups), 0, "Expected 0 query group sizes, i.e., len(qgroups) from prepare_training_data")
    ...
    def test_maximize_mmr_small_lambda_2(self):
        exp_results = [(4, 0), (5, -0.2), (1, -0.4), (2, -0.5)]
        random_search_results = [(4, 7.457237795021482), (1, 3.2321043774997484), (5, 2.2657904308788495), (2, 0.38815195030476646)]
        similarity_matrix = np.array([[1, 0.1, 0.2, 0.3],[0.1, 1, 0.4, 0.3],[0.2, 0.4, 1, 0.5],[0.3, 0.3, 0.5, 1]])
        list_docs = [5,1,4,2]
        lmbda = 0
    ...
    def test_maximize_mmr_large_lambda_2(self):
        exp_results = [(4, 7.457237795021482), (1, 3.2321043774997484), (5, 2.2657904308788495), (2, 0.38815195030476646)]
        random_search_results = [(4, 7.457237795021482), (1, 3.2321043774997484), (5, 2.2657904308788495), (2, 0.38815195030476646)]
        similarity_matrix = np.array([[1, 0.1, 0.2, 0.3],[0.1, 1, 0.4, 0.3],[0.2, 0.4, 1, 0.5],[0.3, 0.3, 0.5, 1]])
        list_docs = [5,1,4,2]
        lmbda = 1
...
class TestL2RRanker_MMR(unittest.TestCase):
    ...
    def test_query_with_mmr_zero(self):
        ...
        self.assertEqual(result1, result2, "Expected same results for mmr_threshold=0 and mmr_lambda=1")
...
class TestL2RFeatureExtractor(unittest.TestCase):
    ...
    def get_noexistent_hub_score(self):
        fe = L2RFeatureExtractor(self.main_index,
                                 self.title_index,
                                 self.doc_category_info,
                                 self.preprocessor,
                                 ...
                                 self.recognized_categories,
                                 self.network_features)
        actual_hub = fe.get_hits_hub_score(100)
        self.assertEqual(actual_hub, 0, msg=f'Expected HITS hub score=0 but got {actual_hub} for document 100')

    def get_noexistent_authority_score(self):
        fe = L2RFeatureExtractor(self.main_index,
                                    self.title_index,
                                    self.doc_category_info,
                                    self.preprocessor,
                                    ...
                                    self.recognized_categories,
                                    self.network_features)
        actual_auth = fe.get_hits_authority_score(100)
        self.assertEqual(actual_auth, 0, msg=f'Expected HITS hub score=0 but got {actual_auth} for document 100')
