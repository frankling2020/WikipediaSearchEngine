...
class TestDoc2QueryAugmenter(unittest.TestCase):
    ...
    def test_query_generation_no_queries(self) -> None:
        d2q = document_preprocessor.Doc2QueryAugmenter('eecs-549/claude-shannon')
        query_count = 0
        queries = d2q.get_queries(self.text, query_count)
        self.assertEqual(len(queries), query_count, msg=f'Expected {query_count} queries as output but received {len(queries)}')
        document_preprocessor.T5ForConditionalGeneration.from_pretrained.assert_called_once_with('eecs-549/claude-shannon')
        document_preprocessor.T5Tokenizer.from_pretrained.assert_called_once_with('eecs-549/claude-shannon')
        self.mockModel.generate.assert_not_called()
        self.mockTokenizer.decode.assert_not_called()

    def test_query_generation_with_single_query(self) -> None:
        d2q = document_preprocessor.Doc2QueryAugmenter('eecs-549/claude-shannon')
        query_count = 1
        self.mockModel.generate.return_value = [0 for _ in range(query_count)]
        queries = d2q.get_queries(self.text, query_count)
    ...
    def test_query_generation_multi_queries(self) -> None:
        d2q = document_preprocessor.Doc2QueryAugmenter('eecs-549/claude-shannon')
        query_count = 10
        self.mockModel.generate.return_value = [0 for _ in range(query_count)]
        queries = d2q.get_queries(self.text, query_count)
    ...
    def test_query_generation_with_prefix_no_queries(self) -> None:
        d2q = document_preprocessor.Doc2QueryAugmenter('eecs-549/claude-shannon')
        query_count = 0
        prefix = 'Some random prompt '
        queries = d2q.get_queries(self.text, query_count, prefix)
    ...
    def test_query_generation_with_empty_document(self) -> None:
        d2q = document_preprocessor.Doc2QueryAugmenter('eecs-549/claude-shannon')
        query_count = 10
        queries = d2q.get_queries('', query_count)
        self.assertEqual(len(queries), 0, msg=f'Expected {query_count} queries as output but received {len(queries)}')
        document_preprocessor.T5ForConditionalGeneration.from_pretrained.assert_called_once_with('eecs-549/claude-shannon')
        document_preprocessor.T5Tokenizer.from_pretrained.assert_called_once_with('eecs-549/claude-shannon')
        self.mockModel.generate.assert_not_called()
        self.mockTokenizer.decode.assert_not_called()
    ...
    def test_query_generation_with_empty_document_and_prefix_no_queries(self) -> None:
        d2q = document_preprocessor.Doc2QueryAugmenter('eecs-549/claude-shannon')
        query_count = 10
        prefix = 'Some random prompt '
        queries = d2q.get_queries('', query_count, prefix)
        self.assertEqual(len(queries), 0, msg=f'Expected {query_count} queries as output but received {len(queries)}')
        document_preprocessor.T5ForConditionalGeneration.from_pretrained.assert_called_once_with('eecs-549/claude-shannon')
        document_preprocessor.T5Tokenizer.from_pretrained.assert_called_once_with('eecs-549/claude-shannon')
        self.mockModel.generate.assert_not_called()
