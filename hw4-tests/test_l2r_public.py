...
class TestL2RFeatureExtractor(unittest.TestCase):
    ...
    def test_get_cross_encoder_score_with_empty_query(self):            
        fe = L2RFeatureExtractor(self.main_index,
                                self.title_index,
                                self.doc_category_info,
                                self.preprocessor,
        ...
        for docid in [1, 2, 3, 4, 5, 6]:
            actual_score = fe.get_cross_encoder_score(docid, '')
            self.assertEqual(actual_score, 0, 
                msg=f'Expected cross encoder score to be 0 but got {actual_score} for document {docid}')
